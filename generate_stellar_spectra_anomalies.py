...
class Params:
    ...
    # poisson_alpha: float = 0.0  # sigma_extra ≈ poisson_alpha * sqrt(flux_clipped)
    ...
    # Weird/out-of-distribution rows (not from low-rank basis)
    weird_frac: float = 0.05  # probability a given spectrum is "weird"
    weird_kinds: tuple[str, ...] = (
        # "emission_spikes",  # galaxy-ish narrow emissions
        "broad_absorption",  # very broad lines (wrong LSF/pop)
        # "ripple_continuum",  # fringing/etalon ripple
        # "featureless_slope",  # continuum only
        # "shift_mismatch",  # huge RV shift outside training
    )
    weird_amplitude: float = 1.0  # global amplitude scaler for weirdness

    seed: int = 42
    out: Path = Path("spectra_anomalies.npz").name
...
def _make_weird_spectra(
    ...
    """Return (Y_weird, labels) with shape (Nw, M) and list[str] labels."""
    M = lam.size
    labels = []
    Yw = np.empty((Nw, M), dtype=float)

    # Precompute handy grids
    loglam = np.log(lam)
    x01 = (lam - lam.min()) / (lam.max() - lam.min())  # in [0,1]
    two_pi = 2.0 * np.pi
    ...
    for i in range(Nw):
        ...
        if kind == "emission_spikes":
            # Random sparse positive spikes on a flat/sloped continuum
            n_lines = rng.integers(5, 25)
            centers = rng.choice(lam, size=n_lines, replace=False)
            widths = rng.uniform(0.2, 1.0, size=n_lines)  # Å
            amps = rng.uniform(0.05, 0.3, size=n_lines) * amp
            cont_slope = rng.normal(0.0, 0.05)
            y = 1.0 + cont_slope * (x01 - 0.5)
            for c, w, a in zip(centers, widths, amps):
                y += a * np.exp(-0.5 * ((lam - c) / w) ** 2)
            Yw[i] = y

        elif kind == "broad_absorption":
            # Few *very* broad troughs + smooth continuum — not representable by your narrow LSF
            n_trough = rng.integers(1, 4)
            centers = rng.uniform(lam.min(), lam.max(), size=n_trough)
            widths = rng.uniform(8.0, 40.0, size=n_trough)  # wide!
            depths = rng.uniform(0.05, 0.25, size=n_trough) * amp
            cont_c = rng.normal(1.0, 0.05)
            cont_q = rng.normal(0.0, 0.03)
            y = cont_c + cont_q * (x01 - 0.5) ** 2
            for c, w, d in zip(centers, widths, depths):
                y += -d * np.exp(-0.5 * ((lam - c) / w) ** 2)
            Yw[i] = y

        elif kind == "ripple_continuum":
            # Low-rank can’t express a high-freq fringe (period varies with λ)
            base = 1.0 + rng.normal(0.0, 0.03) + rng.normal(0.0, 0.03) * (x01 - 0.5)
            f0 = rng.uniform(12.0, 28.0)  # cycles across band
            chirp = rng.uniform(0.5, 1.5)
            phase = rng.uniform(0, two_pi)
            ripple = 0.03 * amp * np.sin(two_pi * (f0 * x01 + chirp * x01**2) + phase)
            Yw[i] = base + ripple

        elif kind == "featureless_slope":
            # Essentially no lines, just continuum shape
            slope = rng.normal(0.0, 0.15)
            quad = rng.normal(0.0, 0.10)
            Yw[i] = 1.0 + slope * (x01 - 0.5) + quad * (x01 - 0.5) ** 2

        elif kind == "shift_mismatch":
            # Same *line* template idea but with absurd RV (well outside training)
            # Use a crude deep Balmer-only absorption set at a huge shift
            centers = np.array([4101.7, 4340.5, 4861.3, 6562.8])
            rv_big = rng.normal(0.0, 350.0)  # km/s, much larger than training
            c_kms = 299792.458
            shifted = centers * (1.0 + rv_big / c_kms)
            depths = rng.uniform(0.08, 0.22, size=centers.size) * amp
            widths = rng.uniform(0.7, 1.8, size=centers.size)
            y = np.ones(M, dtype=float)
            for c, d, w in zip(shifted, depths, widths):
                y += -d * np.exp(-0.5 * ((lam - c) / w) ** 2)
            Yw[i] = y

        else:
            # Fallback: small random smooth function
            coeffs = rng.normal(0, 0.03, size=4)
            basis = np.vstack(
                [np.ones(M), x01 - 0.5, (x01 - 0.5) ** 2, (x01 - 0.5) ** 3]
            )
            Yw[i] = (coeffs @ basis).clip(0.2, 1.8)
...
def generate(params: Params):
    ...
    # ---- Inject weird / OOD spectra (not from the basis) ----
    rng = np.random.default_rng(params.seed + 1)
    weird_mask = rng.random(params.N) < params.weird_frac
    idx_weird = np.where(weird_mask)[0]
    weird_labels = np.array([""] * params.N, dtype="U24")

    if idx_weird.size > 0:
        Yw, labels = _make_weird_spectra(
            idx_weird.size, lam, rng, params.weird_kinds, amp=params.weird_amplitude
        )
        Y_true[idx_weird] = Yw
        weird_labels[idx_weird] = np.array(labels, dtype="U24")

    # (continue exactly as you already do:)
    sig_col = hetero_noise_sigma(params, lam)
    row_scale = (1.0 + 0.05 * rng.standard_normal((params.N, 1))).clip(0.8, 1.2)
    Sigma_base = row_scale * sig_col[None, :]
    Sigma = Sigma_base.copy()
    sigma_poisson_alpha = 0.0
    if params.use_poisson_like:
        sigma_poisson_alpha = float(params.poisson_alpha)
        flux_clip = np.clip(Y_true, 1e-6, None)
    ...
    meta = {
        ...
        "weird_row_mask": weird_mask.astype(np.bool_),
        "weird_row_kind": weird_labels,
    }
...
def main():
    ...
    for i in rng.choice(p.N, size=20, replace=False):
