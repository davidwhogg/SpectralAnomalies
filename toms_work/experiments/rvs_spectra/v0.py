...
for i in rng.choice(similar_specs.shape[0], size=2, replace=False):
...
# Data to fit
Y = np.nan_to_num(similar_specs)
W = np.nan_to_num(1.0 / spec_u_flux[similar_spec_idxs] ** 2)

rhmf = RHMF(rank=5, nsigma=2.0)
...
plt.imshow(rhmf.W, aspect="auto", cmap="viridis", vmin=0, vmax=1)
plt.colorbar()
plt.show()

plt.figure(dpi=100, figsize=[18, 7], layout="compressed")
plt.plot(spec_λ, rhmf.W.sum(axis=0), lw=2)
plt.show()
...
plt.hist(np.sum(rhmf.W < 0.5, axis=1), bins=30, histtype="step", color="C2", lw=2)
...
n_downweighted = np.sum(rhmf.W < 0.5, axis=1)
...
target_dw = np.sum(rhmf.W[target_idx] < 0.5)
...
fitted_target_spec = rhmf.synthesis()[target_idx]
residual_target = rhmf.resid()[target_idx]
fig, ax = plt.subplots(figsize=[18, 7], dpi=100, layout="compressed")
ax.plot(spec_λ, spec_flux[target_spec_idx], lw=1, alpha=1, c="C2", label="Data")
ax.plot(spec_λ, fitted_target_spec, lw=1, alpha=1, c="C3", label="Fit")
# ax.plot(spec_λ, residual_target, lw=1, alpha=1, c="C4", label="Residual")
ax.set_xlabel(r"$\lambda$ [nm]")
ax.set_ylabel("Normalised Flux")
ax.set_ylim(0.7, 1.1)
ax.set_title(f"Gaia DR3 {TARGET_ID}")
ax.legend()
plt.show()
